    ...
    <template slot="body">
      <div class="md-layout justify-content-center">
        <div class="md-layout-item md-size-60 margin-bottom justify-content-center">
          ...
          <div
            class="label-item label-success text-center"
            v-if="newBudget && newBudget > event.totalBudget"
          >
            <h4>Fantastic!</h4>
            <p>This budget is {{ 100 - parseInt( event.totalBudget * 100 / newBudget) }}% higher than average, your event is going to be wild!</p>
          </div>
          <div
            class="label-item label-warning text-center"
            v-if="newBudget && newBudget < event.totalBudget"
          >
            <p>
              <img :src="`${$iconURL}Event Page/warning-circle-gray.svg`" width="20" />
              This budget is {{ 100 - parseInt( newBudget * 100 / event.totalBudget) }}% lower than average for this type of event
            </p>
          </div>
        </div>
      </div>
