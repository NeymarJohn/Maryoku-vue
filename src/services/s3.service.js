...
class S3Service {
    ...
    async fileUpload(file, key) {
        ...
        const result  = await axios.post(`${process.env.SERVER_URL}/uploadFile`, formData, {
            headers: {
    ...
    // old method for uploading file to s3
    // fileUpload1(file, fileName, dirName, ext){
    ...
    async deleteFile(key) {
        const result = await axios.post(`${process.env.SERVER_URL}/removeFile`);

        return result
    }

    // old method for removing file from s3
