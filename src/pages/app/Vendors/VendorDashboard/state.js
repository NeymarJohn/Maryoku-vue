...
const actions = {
  ...
  getProposals({ commit, state }, payload) {
    return new Promise(async (resolve, reject) => {
      ...
      if (payload.pagination) {
        query.page(payload.pagination.page).limit(payload.pagination.limit);
      }
      if (payload.params) {
  ...
  saveVersion({ commit, state }, payload) {
    return new Promise(async (resolve, reject) => {
      const query = new ProposalVersion({
        ...payload.version,
        proposal: new Proposal({ id: payload.proposal.id })
  ...
  saveNegotiation({ commit, state }, payload) {
    return new Promise(async (resolve, reject) => {
      ...
      let res = await query.for(new Proposal({ id: payload.proposal.id })).save();
      resolve(res);
    });
  },
