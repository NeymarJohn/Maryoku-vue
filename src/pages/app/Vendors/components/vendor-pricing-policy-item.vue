<template>
    <div class="rule">
      ...
      <div class="d-flex align-center">
        ...
        <div class="bottom mt-0 ml-70" v-if="item.hasOwnProperty('discount') && item.type == Boolean && item.value">
          ...
          <input
            ...
            @input="setPricePolicy()"
          />
          <span class="ml-10" v-if="item.hasUnit">
            <select class="unit-select ml-10" v-model="item.unit">
              <option v-for="(unit, index) in item.units" :key="index" :value="unit">{{ unit }}</option>
            </select>
          </span>
