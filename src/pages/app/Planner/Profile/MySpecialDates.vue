  ...
  <div style="width:100%">
    <md-card>
      <md-card-header class="md-card-header-icon md-card-header-rose">
        ...
        <h4 class="title profile-title" style="padding-left: 0;">
          My Special Dates
        </h4>
      </md-card-header>
        ...
        <md-table>
          <md-table-row v-for="(item, index) in specialDaysList" :key="index">
            <md-table-cell v-if="item.canDelete">
              <label-edit :text="item.title" :field-name="`title__${index}`" @text-updated-blur="valueChanged" @text-updated-enter="valueChanged" />
            </md-table-cell>
            <md-table-cell v-else>
              {{ item.title }}
            </md-table-cell>
            <md-table-cell class="text-right">
              <label-edit :text="item.dateString" :field-name="`dateString__${index}`" mask="99/99/9999" @text-updated-blur="valueChanged" @text-updated-enter="valueChanged" />
            </md-table-cell>
            <md-table-cell>
              <md-button v-show="item.canDelete" class="md-simple md-red md-just-icon md-xs" @click="removeSpecialDay(index)">
          ...
          <md-table-row>
            <md-table-cell colspan="3">
              <md-button class="md-simple md-red" style="width: 100%;" @click="addSpecialDay">
                <md-icon>add</md-icon>Add another special day
              </md-button>
            </md-table-cell>
          </md-table-row>
        </md-table>
