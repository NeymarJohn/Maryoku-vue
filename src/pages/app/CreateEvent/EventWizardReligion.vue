<template>
  <div class="">
    <div class="container">
      ...
      <selected-value :value="publicEventData.occasion.name" :property="publicEventData.occasion.value" />
      <div class="event-basic-info wizard-religion">
        ...
        <div class="mt-3 holiday-types">
          <div v-for="(type) in holidays" :key="type.value" class="text-left type" :class="{selected:type.selected}" @click="selectedHoldiay=type">
            <md-checkbox v-model="selectedHoldiay" class="md-checkbox-circle md-red" :value="type">
              <span :class="{selected: selectedHoldiay.value == type.value}">{{ type.name }}</span>
            </md-checkbox>
          </div>
        </div>
      </div>
