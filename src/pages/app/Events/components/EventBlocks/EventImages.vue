    ...
    <div class="event-images-list md-layout">
        ...
        <md-card v-for="(image,index) in event.eventPage.images" :key="index" class="md-layout-item md-size-25">
            <md-card-media>
                ...
                <md-button class="md-info md-xs md-just-icon md-round replace-event-image" @click="uploadEventImage(index)" v-if="!readonly">
                    <md-icon>cached</md-icon>
                    <md-tooltip>Replace</md-tooltip>
                </md-button>
                <md-button class="md-danger md-xs md-just-icon md-round delete-event-image" @click="removeEventImage(index)" v-if="!readonly">
        ...
        <md-card v-if="!readonly" v-for="(n,index) in (3 - Math.min(3,event.eventPage.images.length))" :key="index + 1000"  class="md-layout-item md-size-33 default-image-box" :style="`cursor: pointer; background: url('/static/img/default${index+1}.jpg') center center no-repeat; background-size: cover;`">
