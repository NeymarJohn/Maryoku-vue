            ...
            <div class="md-layout-item md-medium-size-60 md-size-60">
              <md-card style="height: auto;">
                <md-card-header class="md-card-header-text md-card-header-warning">
                  ...
                  <md-button v-if="!inviteesExpanded" class="md-info md-md pull-right" style="margin-top: 16px; margin-right: 12px;" :disabled="working || noActions" @click="toggleInviteesForm">
                    Add Invitees
                  </md-button>
                  <md-button v-if="inviteesExpanded" class="md-success md-md pull-right" style="margin-top: 16px; margin-right: 12px;" :disabled="working || noActions" @click="toggleInviteesForm">
                    Done
                  </md-button>
                </md-card-header>
                <md-card-content class="">
                  <vue-element-loading :active="working" spinner="ring" color="#FF547C" />
                  <div v-if="inviteesExpanded" class="md-layout">
                    <div class="md-layout-item">
                      <md-button class="md-purple md-md" style="margin-top: 16px; margin-right: 12px;" :disabled="working || noActions" @click="importInvitees">
                        Import from spreadsheet
                      </md-button>
                      <md-button class="md-info md-md" style="margin-top: 16px; margin-right: 12px;" :disabled="working || noActions" @click="invite">
                        Add manually
                      </md-button>
                    </div>
                  </div>
                  <div class="md-layout md-gutter" style="margin: 0;">
                    ...
                    <div class="md-layout-item md-size-100">
                      <md-field style="border: none;" class="clear-margins">
                        <multiselect :show-no-results="false" :internal-search="false" :reset-after="true" :close-on-select="true" :preserve-search="true" placeholder="" label="emailAddress" track-by="id" :searchable="true" :options="allOptions" :multiple="true" @search-change="searchOptions" @select="selectMember">
                          <template slot="clear" slot-scope="props">
                            <div v-if="allOptions.length" class="multiselect__clear" @mousedown.prevent.stop="clearSearch(props.search)" />
                          </template>
                          <template slot="caret">
                            <span />
                          </template>
                          ...
                          <template slot="option" slot-scope="{option}">
                            ...
                            <div v-if="option.type === 'person'">
                              <div v-if="option.firstName || option.lastName" class="md-menu-item">
                                {{ option.firstName }} {{ option.lastName }} <span class="text-gray">&nbsp;({{ option.emailAddress }})</span>
                              </div>
                              <div v-else-if="option.displayName" class="md-menu-item">
                    ...
                    <div v-if="eventInvitees.length" class="md-layout-item md-size-100">
                      <md-table v-model="groups" class="text-left ">
                        <tr class="md-table-row md-table-head-container">
                          <th class="md-table-head" style="width: 50%;">
                            Group
                          </th>
                          <th class="md-table-head md-numeric md-xs" style="width: 25%;">
                            Invitees
                          </th>
                          <th class="md-table-head md-numeric" style="width: 24%;">
                            RSVP
                          </th>
                          <th class="md-table-head" style="width: 1%;" />
                        </tr>
                        <template v-for="item in groups">
                          <tr class="md-table-row" style="cursor: pointer;" :class="{'visible-row':item.expanded}" @click="expandGroup(item)">
                          ...
                          <tr v-if="item.expanded" class="bg-white" :class="{'visible-row':item.expanded}">
                            <td class="md-table-cell text-center" colspan="4" style="max-width: 1px; padding: 24px;">
                              <event-invitee-group-details :group.sync="item" @unselect-member="unselectMember" />
                            </td>
                          </tr>
                        </template>
                      </md-table>
                    </div>
                    <div v-if="!eventInvitees.length" class="md-layout-item md-size-100 text-center" style="margin-top: 8px;">
                ...
                <md-card-actions>
                  <div class="text-left">
                    {{ eventInvitees.length }} / {{ eventData.numberOfParticipants }} expected invitees
                  </div>
                </md-card-actions>
              </md-card>
            </div>
                ...
                <md-card-content>
                  <div class="text-left">
                    ...
                    <div v-for="(option, index) in InviteeTypes" :key="index">
                      <md-radio v-model="eventData.participantsType" :value="option">
                        {{ option }}
                      </md-radio>
                    </div>
                  </div>
