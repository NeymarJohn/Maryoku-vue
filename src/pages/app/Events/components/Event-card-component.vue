<template>
  <div class="md-layout-item md-size-100">
            ...
            <md-tabs md-sync-route class="md-success" md-alignment="left">
              ...
              <md-tab id="tab-vendors" md-label="Vendors" md-icon="rv_hookup">
                <md-table v-model="componentObject.vendors" table-header-color="green" v-if="componentObject.vendors.length">
                  <md-table-row slot="md-table-row" slot-scope="{ item }" @click.native="showModalVendors({item})">
    ...
    <event-modal-vendor ref="vendorsModal" :vendorItem="changedVendorItem" :componentId="componentObject.id"></event-modal-vendor>
    <event-modal-components ref="componentsModal" :componentItem="componentItem" :componentId="componentObject.id"></event-modal-components>
    <event-modal-todo ref="todoModal" :todoItem="todoItem" :componentId="componentObject.id"></event-modal-todo>
  </div>
</template>
...
<script>
  ...
  export default {
    ...
    methods: {
      ...
      showModalVendors(item) {
        this.changedVendorItem = item;
        this.$refs.vendorsModal.toggleModal(true);
      },
      showModalComponent(item) {
