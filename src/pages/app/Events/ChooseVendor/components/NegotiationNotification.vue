<template>
  <div class="position-relative">
    <p class="text-left my-20">
      These vendors has given you 2 more days to decide
    </p>
    <div v-for="(subProposals, category) in proposals" :key="category" class="mb-30">
      ...
      <div class="d-flex align-center">
        <div v-for="p in subProposals" :key="p.id" class="d-flex align-start mb-10 pr-20">
          <img v-if="image(p)" :src="image(p)" class="mr-10" width="80px">
          <div class="text-left">
            <div class="font-bold-extra">
              {{ p.vendor.companyName }}
            </div>
            <div class="font-size-20 font-bold-extra mt-10">
              ${{ p.cost | withComma(Number) }}
            </div>
          </div>
        </div>
      </div>
    ...
    <VendorBidTimeCounter
      custom-class="vendor bg-red py-10"
      :days="days"
      :hours="hours"
      :minutes="mins"
      :seconds="seconds"
