      ...
      <div class="p-50">
        ...
        <div class="d-flex mt-70 mb-40">
          ...
          <div class="ml-20">
            ...
            <div class="font-size-22 line-height-1">{{ campaign.name }}</div>
            <!-- <title-editor :value="info.conceptName" @change="changeTitle" class="mt-40"></title-editor> -->
          </div>
        </div>
        <maryoku-textarea :placeholder="placeHolder" v-model="campaign.description"></maryoku-textarea>
      </div>
      <feedback-image-carousel class="p-50" :images="campaign.images" @addImage="addNewImage"></feedback-image-carousel>
      <div class="p-50">
        <div class="d-flex align-center font-bold">Allow guests to upload photos form the event</div>
        ...
        <div v-if="campaign.files && campaign.files.length > 1">
          <span class="font-bold">{{ campaign.files[0].name }}</span>
        </div>
        ...
        <div>
          ...
          <div>
            <feedback-question
              v-for="(question, index) in campaign.feedBack"
              :key="index"
