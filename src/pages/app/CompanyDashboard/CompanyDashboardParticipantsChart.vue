  ...
  <md-card>
    <md-card-header class="md-card-header-icon md-card-header-blue">
      ...
      <h5 class="title" style="padding-top: 6px;">
        ...
        <md-field class="pull-right md-xs" style="max-width: 120px; margin: 0; border: none;" md-inline>
          <md-select id="filterMonth" v-model="filterMonth" name="filterMonth" @md-selected="filterMonthChanged($event)">
            <md-option value="01">
              January
            </md-option>
            <md-option value="02">
    ...
    <md-card-content style="min-height: 120px">
      ...
      <template v-for="item in items">
        <div class="bar-item">
          <h5 style="margin-bottom: 0; margin-top: 0; font-weight: 500; font-size: 14px;">
            {{ item.text }}
          </h5>
          <div class="text-gray small" style="padding-bottom: 6px;">
            {{ item.theDate }} <span class="pull-right"><span class="black-color">{{ item.actual }}</span> / {{ item.invited }}</span>
          </div>
          <md-progress-bar class="md-info" style="min-height: 18px;" :md-value="(item.actual/item.invited)*100" />
        </div>
      </template>
      <div v-if="items.length === 0" class="">
        <h6 style="text-align: center;">
          No participants information this month
        </h6>
      </div>
    </md-card-content>
  </md-card>
...
export default {
  name: "CompanyDashboardParticipantsChart",
  components: {
    VueElementLoading
  },
  props: {
