    ...
    <md-table v-model="rfps" class="mt-10">
      <md-table-row slot="md-table-row" slot-scope="{ item }">
        ...
        <md-table-cell md-label="Event Name">
          <div v-if="item.eventData.concept" class="font-bold font-size-16">
            {{ item.eventData.concept.name }}
          </div>
          <div v-else-if="item.eventData.title">
        ...
        <md-table-cell md-label="Date">
          {{ item.eventData.eventStartMillis | date("DD/MM/YYYY") }}
        </md-table-cell>
        <md-table-cell md-label="Expire At">
          {{ item.expiredTime | date("DD/MM/YYYY") }}
        </md-table-cell>
        ...
        <md-table-cell md-label="Cost">
          $ {{ (item.proposal ? item.proposal.cost : item.componentInstance ? item.componentInstance.allocatedBudget : 0) | withComma }}
        </md-table-cell>
        <md-table-cell md-label="Status">
          {{ item.proposal ? 'Submitted' : 'Pending' }}
        </md-table-cell>
      </md-table-row>
    </md-table>
  </div>
