...
const mutations = {
  ...
  setAdditionalServices: (state, services) => {
    ...
    setStateByVersion(state, {
      ...
      value: JSON.parse(JSON.stringify(state.additionalServices))
    });
  },
    ...
    return new Promise((resolve, reject) => {
      ProposalRequest.find(requestId).then((resp) => {
        ...
        if (resp.proposal) {
          ...
          if (resp.proposal.selectedVersion && resp.proposal.selectedVersion > -1)
            commit("selectVersion", resp.proposal.selectedVersion);
        }
        resolve(resp);
      });
    });
