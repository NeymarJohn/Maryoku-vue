...
export default {
  ...
  login(context, creds, successCallback, errorCallback) {
    context.$http.post(SESSION_URL, creds).then(
      (resp) => {
  ...
  signupOrSignin(context, userData, successCallback, failedCallback) {
    context.$http.post(`${REGISTRATION_URL}`, userData, { ContentType: "application/json" }).then(
      (resp) => {
        successCallback(resp.data);
      },
      (resp) => {
  ...
  clientSignupOrSignin(context, email, password, department, callback) {
    context.$http
      .post(
        `${REGISTRATION_URL}`,
        { username: email, password: password, department: department, role: "client" },
  ...
  guestSignupOrSignin(context, details, callback) {
    context.$http
      .post(
        `${REGISTRATION_RSVP_URL}`,
        { username: details.email, password: details.password, role: details.role, eventId: details.eventId },
        { ContentType: "application/json" }
      )
      .then(
        (resp) => {
  ...
  forgotPassword(context, email, successCallback, errorCallback) {
    context.$http.post(`${FORGOT_PASSWORD_URL}`, { email }, { ContentType: "application/json" }).then(
      (resp) => {
        if (callback) {
          successCallback(resp.data);
        }
  ...
  currentTenantUser(context, required, cb) {
    ...
    if (!this.user.authenticated || this.user.id === undefined) {
      context.$http
        .get(CURRENT_TENANT_USER, { maxRedirects: 0, headers: this.getAuthHeader() })
        .then((resp) => {
  ...
  currentUser(context, required, cb) {
    ...
    if (!this.user.authenticated || this.user.id === undefined) {
      context.$http
        .get(CURRENT_USER_URL, { maxRedirects: 0, headers: this.getAuthHeader() })
        .then((resp) => {
  ...
  logout(context, options) {
    context.$http.get(LOGOUT_USER_URL, options).then(
      (data) => {
        window.localStorage.removeItem(TOKEN_KEY);
        this.user = {
          authenticated: false
